...
//                fprintf(stderr, "%s encoded size=%d isKeyFrame=%d\n", __FUNCTION__, pkt.size, isKeyFrame);
                ...
                ///剔除4个字节的起始码
                VideoFramePtr framePtr = std::make_shared<VideoFrame>();
                framePtr->setNalu(pkt.data + 4, pkt.size - 4, naluType);

                AppConfig::gRtpSenderManager->sendVideo(framePtr, node.time);
            }
